...
class ZImageModel(BaseModel):
    ...
    def load_model(self):
        ...
        # Allow overriding TE and VAE paths via config (te_name_or_path / vae_path),
        # fall back to the checkpoint's own subfolders
        te_path = getattr(self.model_config, "te_name_or_path", None) or base_model_path
        tokenizer = Qwen3TextEncoder.load_tokenizer(te_path)
        text_encoder = Qwen3TextEncoder.load(
            te_path, **self.component_load_kwargs("te")
        )

        self.print_and_status_update("Loading VAE")
        vae_source = getattr(self.model_config, "vae_path", None) or base_model_path
        vae = KLVAE.load_model(vae_source, dtype=dtype)
